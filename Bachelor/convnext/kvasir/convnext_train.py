...
pp = Preprocessor(data_io, batch_size=4, subfunctions=sf, data_aug=aug,
                  prepare_subfunctions=True, prepare_batches=False,
                  analysis="fullimage")

 # Create the Neural Network models
convnext_model = Neural_Network(preprocessor=pp, loss=tversky_loss, architecture=convnext,
...
cb_mc_convnext = ModelCheckpoint(filepath=model_path, save_best_only=True)

# Train model and record time
start = time.time()
history = convnext_model.evaluate(training_samples=sample_list, validation_samples=sample_list_val, epochs=1000, callbacks=[cb_lr, cb_tb, cb_es, cb_mc_convnext])
duration = round(time.time() - start)

# save training duration
with open("convnext_unet_training_duration_kvasir.txt", "w") as file:
...
savefig("convnext_unet_fitting_curve_kvasir.png")
