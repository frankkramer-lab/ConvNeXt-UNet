...
from cbam_unet import Architecture as Cbam

# ----------------------------   Paths   ------------------------------------ # 
# Change these paths to where the corresponding data is stored on your system #
# --------------------------------------------------------------------------- #
# Path to test subset of Kvasir-SEG dataset
training_path = "jonas/Bachelor/kvasir_seg/training/data"
# Path to validation subset of Kvasir-SEG dataset
validation_path = "jonas/Bachelor/kvasir_seg/validation/data"
# Path to where the model should be saved after training
model_path = "jonas/Bachelor/cbam/cbam_unet_kvasir.hdf5"
...
cbam_model = Neural_Network(preprocessor=pp, loss=tversky_loss, architecture=cbam,
...
cb_mc_cbam = ModelCheckpoint(filepath=model_path, save_best_only=True)

# Train model and record time
start = time.time()
history = cbam_model.evaluate(training_samples=sample_list, validation_samples=sample_list_val, epochs=1000, callbacks=[cb_lr, cb_tb, cb_es, cb_mc_cbam])
duration = round(time.time() - start)

# save training duration
with open("cbam_unet_training_duration_kvasir.txt", "w") as file:
...
savefig("cbam_unet_fitting_curve_kvasir.png")